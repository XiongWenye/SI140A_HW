...
So in general, the probability that buying $n$ boxes can collect all 108 types of coupons is
$$
\sum_{i=0}^{108} (-1)^{108-i} \left(\begin{array}{c}108 \\ j\end{array}\right) {(\frac{i}{108})}^n
$$
We can write a Python program to calculate the minimum number of $n$ when such probability is no less than $95 \%$ according to the formula above.
\begin{lstlisting}
import numpy as np
import math
...
\begin{center}
    \includegraphics[width=0.8\textwidth]{Coupon Collector Problem.png}
\end{center}
From the graph, we know that with the increasing value of $n$, the probability of getting all 108 coupons increases in a 'S' shape. And we can see that the minimum number of $n$ when such probability is no less than $95 \%$ is 823.
